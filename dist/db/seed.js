...
const markets = ['shopee', 'tokopedia', 'lazada'];
const suppliers = ['1688', 'alibaba', 'aliexpress', 'taobao'];
const stages = ['early', 'rising', 'peak'];
async function seed() {
    ...
    // Generate signals
    const signals = products.flatMap((p) => platforms.map((platform) => {
    ...
    logger_1.default.info(`✅ Inserted ${signals.length} signals`);
    // Generate market validations
    const validations = products.flatMap((p) => markets.map((marketplace) => {
    ...
    logger_1.default.info(`✅ Inserted ${validations.length} market validations`);
    // Generate supplier analyses
    const supplierData = products.flatMap((p) => suppliers.map((supplier_platform) => {
    ...
    logger_1.default.info(`✅ Inserted ${supplierData.length} supplier analyses`);
    ...
    logger_1.default.info(`✅ Inserted ${forecasts.length} forecasts`);
    logger_1.default.info('🎉 Seed complete! Open http://localhost:3000 to view the dashboard.');
    await connection_1.pool.end();
}
seed().catch((err) => {
    logger_1.default.error('Seed failed:', err);
    process.exit(1);
});
