// src/services/AIOpportunityScorer.ts
import {
  TrendSignal, TrendMarketValidation, TrendSupplierAnalysis,
  TrendOpportunity, Platform, TrendStage,
} from '../types/trend.types';
import { GeminiForecaster } from './GeminiForecaster';
import { config } from '../config';
import logger from '../utils/logger';

export class AIOpportunityScorer {
  private forecaster = new GeminiForecaster();
  ...
  async score(
    signals:     TrendSignal[],
    validations: TrendMarketValidation[],
    suppliers:   TrendSupplierAnalysis[],
  ): Promise<TrendOpportunity[]> {
    const keywords = [...new Set(signals.map((s) => s.keyword))];
    logger.info(`[AIScorer] Scoring ${keywords.length} keywords...`);

    const results = await Promise.allSettled(
      keywords.map((kw) => this.scoreKeyword(kw, signals, validations, suppliers)),
    );

    const opportunities = results
      .filter((r): r is PromiseFulfilledResult<TrendOpportunity | null> => r.status === 'fulfilled')
      .map((r) => r.value)
      .filter((o): o is TrendOpportunity => o !== null)
      .sort((a, b) => b.trend_opportunity_score - a.trend_opportunity_score);

    logger.info(`[AIScorer] Scored ${opportunities.length} opportunities`);
    return opportunities;
  }

  private async scoreKeyword(
    keyword:     string,
    signals:     TrendSignal[],
    validations: TrendMarketValidation[],
    suppliers:   TrendSupplierAnalysis[],
    ...
    if (!kwSignals.length) return null;
    ...
    const trend_opportunity_score = this.calculateOpportunityScore(
      virality_score, demand_score, margin_potential, supplier_feasibility_score,
    );

    // Detect top platform
    const platformCounts: Record<string, number> = {};
    kwSignals.forEach((s) => { platformCounts[s.platform] = (platformCounts[s.platform] || 0) + 1; });
    const top_platform = (Object.entries(platformCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || 'tiktok') as Platform;

    // Detect trend stage (use highest stage)
    const stageOrder: Record<TrendStage, number> = { early: 0, rising: 1, peak: 2 };
    const trend_stage = kwSignals.reduce((best, s) =>
      stageOrder[s.trend_stage] > stageOrder[best] ? s.trend_stage : best,
      'early' as TrendStage,
    );

    // Check auto-push thresholds
    const { trendScoreThreshold, marginThresholdPct, competitionThreshold } = config.scoring;
    const auto_pushed =
      trend_opportunity_score > trendScoreThreshold &&
      margin_potential        > marginThresholdPct &&
      competition_score       < competitionThreshold;

    const base: Omit<TrendOpportunity, 'forecast'> = {
      keyword,
    ...
    if (auto_pushed) {
      logger.info(`[AIScorer] 🚀 AUTO-PUSHED: "${keyword}" (score: ${trend_opportunity_score.toFixed(1)})`);
    }
